...
def test_column_filtering_changes_edit_url_on_the_fly(
    ...
    assert driver.current_url == f"{LECTURE_URL}?amendement=666"
    see_td = driver.find_element_by_css_selector("td:nth-child(7)")
    see_link = see_td.find_element_by_css_selector("a")
    assert see_link.get_attribute("href") == f"{LECTURE_URL}/666/amendement_edit"
    see_link.click()
    assert driver.current_url == (
        f"{LECTURE_URL}/666/amendement_edit?"
