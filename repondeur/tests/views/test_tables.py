...
def test_tables_with_amendement(app, lecture_an, amendements_an, user_david):
    ...
    with transaction.manager:
        DBSession.add(user_david)
        user_david.table.amendements.append(amendements_an[0])
...
def test_tables_grab_amendement(app, amendements_an, user_david):
    from zam_repondeur.models import DBSession, User

    with transaction.manager:
        DBSession.add(user_david)
        assert len(user_david.table.amendements) == 0
    ...
    assert len(user_david.table.amendements) == 1
    assert user_david.table.amendements[0].num == amendements_an[0].num
    assert user_david.table.amendements[0].lecture == amendements_an[0].lecture
    assert len(user_david.table.amendements[0].events) == 1
    assert user_david.table.amendements[0].events[0].render_summary() == (
        "<abbr title='david@example.com'>David</abbr> "
        "a mis l’amendement sur sa table"
    )


def test_tables_release_amendement(app, amendements_an, user_david):
...
def test_tables_transfer_amendement(app, amendements_an, user_david, user_ronan):
    from zam_repondeur.models import DBSession, User
    ...
    with transaction.manager:
        ...
        user_david.table.amendements.append(amendements_an[0])
        assert len(user_david.table.amendements) == 1
        assert len(user_ronan.table.amendements) == 0
    ...
    assert len(user_david.table.amendements) == 0
    assert len(user_ronan.table.amendements) == 1
    assert user_ronan.table.amendements[0].num == amendements_an[0].num
    assert user_ronan.table.amendements[0].lecture == amendements_an[0].lecture
    assert len(user_ronan.table.amendements[0].events) == 1
    assert user_ronan.table.amendements[0].events[0].render_summary() == (
        "<abbr title='david@example.com'>David</abbr> "
        "a transféré l’amendement à « Ronan (ronan@example.com) »"
    )


def test_tables_steal_amendement(app, amendements_an, user_david, user_ronan):
    from zam_repondeur.models import DBSession, User
